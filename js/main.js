...
async function p1_sendPacket() {
  ...
  sending = 0;
  if (!success) {
    logEntry("Sender's packet did not reach receiver");
    return;
  } else {
    logEntry("Sender's packet reached receiver");
  }
  ...
  receiving = 0;
  if (!returnSuccess) {
    logEntry("Receiver's packet did not reach sender");
    return;
  } else {
    logEntry("Receiver's packet reached sender");
    countACK++;
  }
  if (countACK == 2) {
